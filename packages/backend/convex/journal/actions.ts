...
async function generateJournalPdf(
	...
	const chosen = selected
		...
		: allEntries;
	// Only written entries can be printed: recorded audio and the withdrawn
	// video entries have no page representation now that the PDF carries no QR.
	const entries = chosen.filter((e) => e.mode === "writing");

	if (entries.length === 0) {
		throw new ConvexError({
			code: "NO_ENTRIES",
			message:
				"Only written entries can be exported. Add a writing entry to build your book.",
		});
	}

	// Oldest-first reads more naturally in a printed book.
	const ordered = [...entries].sort((a, b) => a.dateMs - b.dateMs);
	const journalEntries = mapEntriesForDocugenerate(ordered);
